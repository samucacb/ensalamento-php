        ...
        <div class="row mb-4">
            ...
            <div class="col-auto">
                <button type="button" class="btn btn-primary" onclick="novaSala()">
                    ...
                    <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#modalImportacao">
                <i class="bi bi-upload"></i> Importar Salas
                </button>
            </div>
        </div>


        <!-- Em salas.php, no final do arquivo -->

<!-- Modal de Importação de Salas -->
<div class="modal fade" id="modalImportacao" tabindex="-1" aria-labelledby="modalImportacaoLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalImportacaoLabel">Importar Salas em Massa</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
            </div>
            <div class="modal-body">
                <p>Faça o upload de um arquivo Excel (.xlsx) ou CSV (.csv) para cadastrar múltiplas salas de uma só vez.</p>
                
                <div class="mb-3">
                <a href="https://docs.google.com/spreadsheets/d/1nTxYAZnl1ezsTz9FXZc6WWH0-G3_xTo9/edit?usp=sharing&ouid=111425278833118634287&rtpof=true&sd=true" class="btn btn-sm btn-outline-primary" download target="_blank">
                    <i class="bi bi-download"></i> Baixar Modelo (Template)
                </a>
                </div>
                <form id="formImportacao" enctype="multipart/form-data">
                    <div class="mb-3">
                        <label for="arquivo-salas" class="form-label">Selecione o arquivo</label>
                        <input class="form-control" type="file" id="arquivo-salas" name="arquivo_salas" accept=".xlsx, .csv" required>
                    </div>
                </form>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" onclick="executarImportacao()">
                    <i class="bi bi-check-circle"></i> Enviar e Processar
                </button>
            </div>
        </div>
    </div>
        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <input type="text" class="form-control" id="filtro-predio" placeholder="Nome do prédio" onkeyup="filtrarSalas()">
            </div>
            <div class="col-md-2">
                <label class="form-label">Bloco</label>
                <input type="text" class="form-control" id="filtro-bloco" placeholder="Nome do bloco" onkeyup="filtrarSalas()">
            </div>
                    <div class="col-md-2">
                    ...
                    <table class="table table-striped table-hover" id="tabela-salas">
                        <thead>
                            <tr>
                                <th>Código</th>
                                <th>Nome</th>
                                <th>Prédio</th> <!-- <<-- ADIÇÃO 1 -->
                                <th>Bloco</th>  <!-- <<-- ADIÇÃO 2 -->
                                <th>Tipo</th>
                                <th>Capacidade</th>
                                <th>Localização</th>
                                <th>Status</th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formSala">
                        ...
                        <!-- DENTRO DO SEU FORMULÁRIO EM salas.php -->

<!-- ... (campo de Código, Nome, etc.) ... -->

<div class="row">
    <div class="col-md-6">
        <div class="mb-3">
            <label for="sala-predio" class="form-label">Prédio</label>
            <input type="text" class="form-control" id="sala-predio" name="predio" placeholder="Ex: Prédio A, Principal">
        </div>
    </div>
    <div class="col-md-6">
        <div class="mb-3">
            <label for="sala-bloco" class="form-label">Bloco</label>
            <input type="text" class="form-control" id="sala-bloco" name="bloco" placeholder="Ex: Bloco 1, Anexo">
        </div>
    </div>
</div>

<!-- ... (campo de Capacidade, Tipo, etc.) ... -->
...
function exibirSalas(listaSalas) {
    ...
    if (listaSalas.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="8" class="text-center text-muted"> <!-- <<-- Colspan atualizado para 8 -->
    ...
    listaSalas.forEach(sala => {
        ...
        // =================================================================
        // >> CORREÇÃO PRINCIPAL AQUI: Adicionar as novas colunas <td> <<
        // =================================================================
        html += `
            <tr>
                <td><strong>${sala.codigo}</strong></td>
                <td>${sala.nome}</td>
                <td>${sala.predio || '-'}</td>      <!-- <<-- ADIÇÃO 1 -->
                <td>${sala.bloco || '-'}</td>       <!-- <<-- ADIÇÃO 2 -->
                <td>${tipoBadge}</td>
...
function executarImportacao() {
    ...
    if (!formData.get('arquivo_salas').name) {
        resultadoDiv.innerHTML = '<div class="alert alert-danger">Por favor, selecione um arquivo.</div>';
        return;
    }

    // Mostra um feedback de carregamento
    resultadoDiv.innerHTML = '<div class="alert alert-info">Enviando e processando o arquivo... Por favor, aguarde.</div>';

    fetch('api/salas.php?action=importar', {
        method: 'POST',
        ...
        if (data.success) {
            let mensagem = `
                <div class="alert alert-success">
                    ...
                    - Salas processadas: ${data.data.total_linhas}  

                    - Salas criadas com sucesso: ${data.data.sucessos}  

                    - Salas com erro ou já existentes: ${data.data.erros}
                </div>
            `;
            if (data.data.erros > 0 && data.data.mensagens_erro.length > 0) {
                mensagem += `
                    <p><strong>Detalhes dos erros:</strong></p>
                    <ul>
                        ${data.data.mensagens_erro.map(e => `<li>${e}</li>`).join('')}
                    </ul>
                `;
            }
            resultadoDiv.innerHTML = mensagem;
            carregarSalas(); // Atualiza a lista de salas na página principal
        } else {
            resultadoDiv.innerHTML = `<div class="alert alert-danger">${data.message}</div>`;
        }
    })
    .catch(error => {
        resultadoDiv.innerHTML = `<div class="alert alert-danger">Erro na comunicação: ${error.message}</div>`;
    });
}
        ...
        function atualizarEstatisticas(listaSalas) {
            ...
            const ativas = listaSalas.filter(s => s.ativo == 1 || s.ativa == 1).length;
        ...
        function filtrarSalas() {
            ...
            const predio = document.getElementById('filtro-predio').value.toLowerCase(); // << NOVO
            const bloco = document.getElementById('filtro-bloco').value.toLowerCase(); 
            ...
            let salasFiltradas = salas.filter(sala => {
                ...
                if (predio && (!sala.predio || !sala.predio.toLowerCase().includes(predio))) {
                return false;
                }
                if (bloco && (!sala.bloco || !sala.bloco.toLowerCase().includes(bloco))) {
                return false;
                }
        ...
        function limparFiltros() {
            ...
            document.getElementById('filtro-predio').value = '';
            document.getElementById('filtro-bloco').value = '';
        ...
        function editarSala(id) { 
            ...
            document.getElementById('sala-predio').value = sala.predio || '';
            document.getElementById('sala-bloco').value = sala.bloco || '';
...
function confirmarExclusao() {
    ...
    // Crie um objeto FormData para enviar os dados como um formulário
    const formData = new FormData();
    formData.append('id', salaParaExcluir);

    fetch(`api/salas.php?action=excluir`, { // Remova o ID da URL
        method: 'POST', // 1. Altere o método para POST
        body: formData  // 2. Envie o ID no corpo da requisição
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            ...
            // A linha abaixo agora mostrará o erro vindo do PHP
