<?php
// No topo de ensalamento.php

// Inclua o arquivo de conexão se ele ainda não estiver incluído
require_once __DIR__ . '/config/database.php';

// 1. Busca a lista de todos os períodos disponíveis na nova tabela 'periodos'
$stmt_periodos = $database->prepare("SELECT periodo FROM periodos ORDER BY periodo DESC");
...
//    Na página de ensalamento, faz sentido que o período mais recente seja o padrão.
$periodo_selecionado = $_GET['periodo'] ?? ($periodos_disponiveis[0] ?? '');
...
// Em ensalamento.php (ou seu .js)

// SUBSTITUA a sua função carregarHistorico por esta:
function carregarHistorico() {
    // 1. Lê o período do mesmo seletor que a carregarStatusSistema usa.
    const periodoEl = document.getElementById('periodo');
    if (!periodoEl) {
        console.error("Elemento do seletor de período com ID 'periodo' não encontrado.");
        return;
    ...
    const periodo = periodoEl.value;
        if (!periodo) {
        console.warn("Nenhum período selecionado. A chamada para carregar o histórico foi abortada.");
        // Opcional: Limpar a exibição do histórico
        exibirHistorico([]); // Chama a função com um array vazio para mostrar a mensagem padrão.
        return; // Interrompe a função.
    }

    // 2. Usa a variável 'periodo' na chamada da API.
    fetch(`api/relatorios.php?action=historico&periodo=${periodo}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // A sua função de exibição já está correta.
                exibirHistorico(data.data); 
                
                // ATUALIZA OS CARDS DE "ÚLTIMO ENSALAMENTO" E "EFICIÊNCIA"
                if (data.data && data.data.length > 0) {
                    const ultimo = data.data[0]; // Pega o registro mais recente
                    document.getElementById('ultimo-ensalamento').textContent = new Date(ultimo.data_execucao + 'T00:00:00').toLocaleDateString('pt-BR');
                    document.getElementById('eficiencia-anterior').textContent = parseFloat(ultimo.eficiencia_media || 0).toFixed(1) + '%';
                } else {
                    document.getElementById('ultimo-ensalamento').textContent = 'Nunca';
                    document.getElementById('eficiencia-anterior').textContent = '0.0%';
                }
