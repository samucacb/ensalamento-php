...
function carregarStatusSistema() {
    ...
    // pois o seletor está dentro de um modal e pode não estar acessível.
    const periodoPadrao = '2025.1'; 

    fetch(`api/relatorios.php?action=status&periodo=${periodoPadrao}`) 
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                exibirStatus(data.data);
            } else {
                console.error('Erro ao carregar status:', data.message || 'Resposta sem sucesso da API.');
            }
        })
        .catch(error => {
...
*/
        ...
        function exibirResultados(resultado) {
            ...
            document.getElementById('resultado-conflitos').textContent = resultado.turmas_conflito || 0;
            document.getElementById('resultado-eficiencia').textContent = (resultado.eficiencia_media || 0).toFixed(1) + '%';
            document.getElementById('resultado-tempo').textContent = resultado.tempo_processamento || 0;
            ...
            if (resultado.conflitos && resultado.conflitos.length > 0) {
                let html = '<ul class="list-unstyled">';
                resultado.conflitos.slice(0, 3).forEach(conflito => {
                    html += `<li><i class="bi bi-exclamation-triangle text-warning me-2"></i>${conflito}</li>`;
                });
                html += '</ul>';
                conflitosDiv.innerHTML = html;
            } else {
