...
require_once __DIR__ . '/config/database.php'; // Esta linha já cria o objeto $database.

// 1. Busca a lista de TODOS os períodos usando o nome de coluna correto.
$sql_todos = "SELECT periodo FROM periodos ORDER BY periodo DESC"; // CORRIGIDO
$stmt_todos = $database->prepare($sql_todos);
$stmt_todos->execute();
$periodos_disponiveis = $stmt_todos->fetchAll(PDO::FETCH_COLUMN);

// 2. Determina qual período deve vir selecionado.
$periodo_selecionado = $_GET['periodo'] ?? null;

if (!$periodo_selecionado) {
    // Se não veio na URL, busca o período ATIVO usando o nome de coluna correto.
    $sql_ativo = "SELECT periodo FROM periodos WHERE ativo = 1 LIMIT 1"; // CORRIGIDO
    $stmt_ativo = $database->prepare($sql_ativo);
    $stmt_ativo->execute();
    $periodo_selecionado = $stmt_ativo->fetchColumn();
}

// 3. Como última alternativa, se não houver período ativo, pega o mais recente.
if (!$periodo_selecionado && !empty($periodos_disponiveis)) {
    $periodo_selecionado = $periodos_disponiveis[0];
}
?>
...
// DENTRO DE relatorios.js (ou similar)

// >> SUBSTITUA A SUA FUNÇÃO atualizarRelatorios INTEIRA POR ESTA <<
function atualizarRelatorios() {
    const periodo = document.getElementById('periodo-select').value;
    // A linha 'const tipo' foi removida.
    ...
    const filtros = {
        periodo: periodo,
        // A propriedade 'tipo' foi removida.
        curso: curso,
        tipo_sala: tipoSala,
        status: status
    };
...
function atualizarIndicadores(metricas) {
    ...
    const taxaEl = document.getElementById('taxa-alocacao');
    const progressAlocacaoEl = document.getElementById('progress-alocacao');
    if (taxaEl && progressAlocacaoEl) {
        taxaEl.textContent = taxaAlocacao.toFixed(1) + '%';
        progressAlocacaoEl.style.width = taxaAlocacao + '%';
    }
    
    // Tenta atualizar a Utilização de Salas (agora de forma segura)
    const utilizacaoEl = document.getElementById('utilizacao-salas');
    const progressUtilizacaoEl = document.getElementById('progress-utilizacao');
    if (utilizacaoEl && progressUtilizacaoEl) {
        // Se o elemento existir, ele será atualizado.
        // Se não existir, este bloco é simplesmente ignorado, sem gerar erro.
        utilizacaoEl.textContent = utilizacaoSalas.toFixed(1) + '%';
        progressUtilizacaoEl.style.width = utilizacaoSalas + '%';
    }
    
    // Atualiza a Eficiência Geral (agora será executado sem problemas)
    const eficienciaEl = document.getElementById('eficiencia-geral');
    const progressEficienciaEl = document.getElementById('progress-eficiencia');
    if (eficienciaEl && progressEficienciaEl) {
        eficienciaEl.textContent = eficienciaGeral.toFixed(1) + '%';
        progressEficienciaEl.style.width = eficienciaGeral + '%';
    }
}
...
// Em relatorios.js (ou <script> em relatorios.php)

// >> SUBSTITUA A SUA FUNÇÃO exibirAnaliseDeConflitos INTEIRA POR ESTA <<
function exibirAnaliseDeConflitos(conflitos) {
    ...
    conflitos.forEach(item => {
        // =================================================================
        // >> CORREÇÃO FINAL AQUI <<
        // Adicionamos o nome do curso em uma nova linha com a tag <small>.
        // =================================================================
        html += `
            <tr>
                <td>
                    <strong>${item.sala_codigo}</strong>  

                    <small class="text-muted">${item.sala_nome}</small>
                </td>
                <td>
                    <strong>${item.turma1_codigo}</strong> - ${item.turma1_nome}  

                    <small class="text-muted">${item.turma1_curso || 'Curso não informado'}</small>
                </td>
                <td>
                    <strong>${item.turma2_codigo}</strong> - ${item.turma2_nome}  

                    <small class="text-muted">${item.turma2_curso || 'Curso não informado'}</small>
                </td>
                <td>${item.dias}</td>
                <td>${item.horario}</td>
            </tr>
